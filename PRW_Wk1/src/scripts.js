...
window.addEventListener('load', function() {
    ...
    request.onreadystatechange = function() {
        ...
        for(let i = 0; i < responseObject.recipes.length; i++){
    ...
    let savedRecipes = document.querySelector('#saved');
...
function addRecipe() {
    ...
    $("#newRecipes").append("<article class='recipe'><h4 class='recipeHeader'>" +
        $("#recipeName").val() + "</h4>" + "<br/>" +
            "<p>" + $("#recipeDescription").val() + "</p>" + "<br/>" +
            "<p>Category: " + $("#recipeCategory").val() + "</p>" + "<br/>" +
            "<p>Star Rating: " + $("input[name='rating']:checked").val() + "</p>" +
            '<i class="fa fa-trash" onclick="assignDelete(this);" aria-hidden="true"></i></article>'
    );
...
function update() {
    if(validate()){
        if($("#recipeName, #recipeDescription, #recipeCategory, #rating").val() != null
            && $("#recipeName, #recipeDescription, #recipeCategory, #rating").val() != "") {
            addRecipe();
            formClear();
            $("#recipeName").focus();
        }
